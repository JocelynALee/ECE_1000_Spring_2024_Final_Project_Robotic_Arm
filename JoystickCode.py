#Configure Interpreter->Update Micropython->Raspberry Pi Pico W / Pico WH->Install
...
adc_y_joystick = ADC(Pin(27)) #sets the pin 27 to the y varaible
...
servo_yR = PWM(Pin(4), freq=50)
servo_yL = PWM(Pin(2), freq=50)
...
while True:
    ...
    xrange_of_values=math.sqrt(x_value**2)
    yrange_of_values=math.sqrt(y_value**2)
    # Increase or decrease from 8 to get the joystick to read 0 at the middle position
    if xrange_of_values<=8:
        x_value=0
    if yrange_of_values<=8:
        y_value=0
    ...
    yL_duty = get_servo_duty_cycle(y_value, 0.5, 2.5, 20, -100, 100)
    yR_duty = get_servo_duty_cycle(y_value, 0.5, 2.5, 20, 100, -100)
    ...
    if sw_status == 0:
        sw_duty = int((2.5*65535)/20) #press = close
    else:
        sw_duty = int((.5*65535)/20) #by default more open
    
    # Now, update the servo motors
    servo_x.duty_u16(x_duty)
    servo_yR.duty_u16(yR_duty)  
    servo_yL.duty_u16(yL_duty)
    ...
    print(f"y_value is: {y_value},  yR_duty is: {yR_duty}, yL_duty is: {yL_duty}, sw_duty is: {sw_duty}")
